...
class Test_TransactionRepositoryMock:
    ...
    def test_delete_transaction(self):
        repo = TransactionRepositoryMock()
        transaction_expected_to_be_deleted = repo.get_transaction(
    ...
    def test_delete_transaction_not_found(self):
        repo = TransactionRepositoryMock()
        transaction = repo.delete_transaction(
            transaction_id=self.NOT_FOUND_TRANSACTION_ID
        )
        assert transaction is None
    ...
    def test_update_transaction(self):
        ...
        assert transaction_updated.type_value == TransactionTypeEnum.DEPOSIT
        assert transaction_updated.value == 500.0
        assert transaction_updated.current_balance == 1000.0
        assert transaction_updated.timestamp == 5
