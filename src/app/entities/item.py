...
class Item:
    ...
    def __eq__(self, other):
        return (
            ...
            and self.admin_permission == other.admin_permission
        )
...
class PydanticItem(BaseModel):
    item_id: uuid.UUID = Field(
        description="ID único do item, em formato UUID", default_factory=uuid.uuid4
    )
    name: str = Field(
    ...
    admin_permission: bool = Field(
        description="Bool indicando permissão de administador", default=False
    )

    model_config = ConfigDict(
        use_enum_values=False, extra="forbid", populate_by_name=True
    )
